...
namespace MyBanker
{
    public class VisaDankort : Card, INonDomesticTransaction, ICreditCard, ITransactionLimit, IExpiryDate
    {
        public int CreditLimit { get; set; }
        public int TransactionLimit { get; set; }
        public string ExpiryDate { get; set; }

        public VisaDankort(Customer customer) : base(customer)
        {
            RequiredAge = 18;
            Prefixes = new List<int>() { 4 };
        ...
        {
            if ((this.Customer.Account.SpentThisMonth + amount <= TransactionLimit) && (amount <= this.Customer.Account.Balance + CreditLimit))
            {
